...
export function CTA() {
  return (
    <section id="cta" className="relative py-24 sm:py-40 px-6 sm:px-8 w-full flex flex-col items-center overflow-hidden">
      {/* アクセントカラーの暖かみのある背景 */}
      ...
      >
        ...
        {/* ラベル */}
        <p className="text-xs font-medium tracking-[0.4em] text-accent mb-6 z-10">JOIN US</p>

        <h2 className="text-[clamp(1.875rem,5vw,3.5rem)] font-light tracking-wide text-text-primary mb-6 z-10 leading-snug">
          自治の未来へ。
        </h2>
        <p className="text-[clamp(0.9375rem,2vw,1.125rem)] font-light text-text-primary/65 mb-10 sm:mb-12 max-w-lg z-10 leading-loose">
          次世代自治共創会議（すずかん田園フォーラム）に参加し、
          持続可能な地域社会への歩みを共に始めましょう。
        </p>
      ...
      >
        <div className="border-t border-primary/10 pt-12 sm:pt-16 grid grid-cols-1 sm:grid-cols-2 gap-12 sm:gap-16">
          ...
          <div className="flex flex-col gap-5">
            <h3 className="text-[10px] font-medium text-primary tracking-[0.4em] uppercase">主催</h3>
          ...
          {/* 共催 */}
          <div className="flex flex-col gap-5">
            <h3 className="text-[10px] font-medium text-primary tracking-[0.4em] uppercase">共催</h3>
            <div className="grid grid-cols-2 gap-x-6 gap-y-3">
              {["懐徳会", "株式会社懐徳総合研究所", "株式会社パブリックテクノロジーズ", "若手町村長会"].map((org) => (
                <p key={org} className="text-sm sm:text-base font-light text-text-primary/55 leading-snug">
                  {org}
                </p>
              ))}
            </div>
